...
public class ShopCastleHpUpgradeUI : MonoBehaviour
{
    ...
    [Header("Castle HP Upgrade")]
    [SerializeField] private int maxCastleHpLevel = 50;
    ...
    private void LoadData()
    {
        ...
        castleHpLevel = PlayerPrefs.GetInt(CastleHpLevelKey, 0);
    }

    private int GetCost()
    {
        return baseCost + castleHpLevel * costIncreasePerLevel;
    }
    ...
    private void BuyUpgrade()
    {
        ...
        if (castleHpLevel >= maxCastleHpLevel)
        {
        ...
        castleHpLevel = Mathf.Clamp(castleHpLevel, 0, maxCastleHpLevel);

        PlayerPrefs.SetInt(TotalGoldKey, totalGold);
        PlayerPrefs.SetInt(CastleHpLevelKey, castleHpLevel);
        PlayerPrefs.Save();
    ...
    private void RefreshUI()
    {
        ...
        int cost = GetCost();
        bool isMax = castleHpLevel >= maxCastleHpLevel;
        ...
        if (buttonText != null)
        {
            ...
            if (isMax)
            {
                buttonText.text =
                    "<b>HP zamku</b>" +
                    "<space=1.4em>" +
                    "Lv. " + castleHpLevel + "/" + maxCastleHpLevel +
                    "<space=1.4em>" +
